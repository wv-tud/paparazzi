...
#define CV_AE_AWB_TARGET_BRIGHT 0.085
#endif
...
#define awb_muK(x) ( awb_muAbs(x) >= ( 0.8 ) ? ( 2 * awb_muSign( x ) ) : ( awb_muAbs( x ) >= ( 0.05 ) ? awb_muSign( x ) : ( 0 )) )
...
struct image_t* cv_ae_awb_periodic(struct image_t* img) {
    ...
    if(isp_get_statistics_yuv(&yuv_stats) == 0 && yuv_stats.nb_valid_Y > 0.5 * img->h * img->w) {
        ...
        if(yuv_stats.awb_nb_grey_pixels > 0){
            ...
            if((mt9f002.target_exposure > 1.1 * mt9f002.real_exposure) && !gains_maxed)
            {
                mt9f002.gain_blue     += mt9f002.target_exposure / mt9f002.real_exposure * awb_mu;
                mt9f002.gain_red      += mt9f002.target_exposure / mt9f002.real_exposure * awb_mu;
                mt9f002.gain_green1   += mt9f002.target_exposure / mt9f002.real_exposure * awb_mu;
                mt9f002.gain_green2   += mt9f002.target_exposure / mt9f002.real_exposure * awb_mu;
