...
int configure_isp(int video_fd)
{
...
#define AVI_CONV_CLIPS(_rymin, _rymax,                                  \
	...
	if(open_isp_fd(&isp_ctx, video_fd) < 0)
		return -1;

	avi_isp_chain_bayer_inter_set_registers(&isp_ctx, &bayer_inter);
	avi_isp_vlformat_32to40_set_registers(&isp_ctx, &vlformat_32to40);
	avi_isp_bayer_set_registers(&isp_ctx, &bay);
	avi_isp_color_correction_set_registers(&isp_ctx, &cc);
	avi_isp_vlformat_40to32_set_registers(&isp_ctx, &vlformat_40to32);
	avi_isp_chroma_set_registers(&isp_ctx, &chroma);
