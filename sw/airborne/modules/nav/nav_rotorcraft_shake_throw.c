...
float nav_rotorcraft_shake_throw_shake_threshold = 1.5;
float nav_rotorcraft_shake_throw_fall_threshold = 4.5; ///< The motors are spinning so more noise on sensor

float nav_rotorcraft_shake_throw_upside_z_threshold = 8.0;
float nav_rotorcraft_shake_throw_upside_xy_threshold = 1.5;

uint16_t nav_rotorcraft_shake_throw_shake_time = 8;
uint16_t nav_rotorcraft_shake_throw_cancel_time = 512;
uint16_t nav_rotorcraft_shake_throw_timeout_time = 16;
uint16_t nav_rotorcraft_shake_throw_fall_time = 8;
...
void nav_rotorcraft_shake_throw_init( void ) {
    ...
    for(i = 0; i < NB_BLOCK; i++){
        if(!strcmp(NAV_RC_SHAKE_THROW_RESET_BLOCK,flight_blocks[i])){
            block_reset = i;
        }
        if(!strcmp(NAV_RC_SHAKE_THROW_GOTO_BLOCK,flight_blocks[i])){
            block_goto = i;
        }
    }
}
...
void nav_rotorcraft_shake_throw_run( void ) {
    ...
    case NAV_RC_ST_LAUNCHED : {
        ...
        if(nav_block == block_reset){
            nav_rotorcraft_shake_throw_status = NAV_RC_ST_WAITING; ///< We've landed, reset the status
        }
        break;
    }
    default :
